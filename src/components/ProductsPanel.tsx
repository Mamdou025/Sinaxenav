      ...
      {items.map((item) => (
        <a key={item.t} href="#" className="group rounded-xl border border-slate-200 p-4 hover:border-slate-300 hover:bg-slate-50 transition">
          <div className="flex items-start gap-3">
            <Icon name={item.icon} className="h-6 w-6 text-black mt-0.5" />
            <div>
              <div className="text-base font-semibold">{item.t}</div>
              <div className="text-base text-slate-500 mt-1 leading-snug">{item.d}</div>
              <div className="mt-3 text-base font-medium text-slate-900 opacity-0 group-hover:opacity-100 transition">Learn more →</div>
            </div>
          </div>
        </a>
      ))}
